...
const { resolveLeague } = require('./config');
...
function parseArgs(argv) {
  ...
  let noMatchup = false;
  let manualBoosts = null; // null = auto, [] = parsed from --boost
  const projectedAdds = [];

  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--week') {
      const val = args[++i];
      if (!val || isNaN(parseInt(val))) { console.error('--week requires a number'); process.exit(1); }
      ...
      const val = args[++i];
      if (!val || isNaN(parseInt(val))) { console.error('--adds-used requires a number'); process.exit(1); }
      addsUsed = parseInt(val);
      addsUsedExplicit = true;
    } else if (args[i] === '--goalies') {
      includeGoalies = true;
    } else if (args[i] === '--no-matchup') {
      noMatchup = true;
    } else if (args[i] === '--boost') {
      const val = args[++i];
      if (!val) { console.error('--boost requires comma-separated stat names (e.g. HIT,BLK,SOG)'); process.exit(1); }
      manualBoosts = val.split(',').map(s => s.trim().toUpperCase());
    } else if (args[i] === '--add') {
      const raw = args[++i];
      ...
      // Treat as league alias
      parsedLeagueKey = args[i];
    }
  }

  return { leagueKey: parsedLeagueKey, targetDate, addsUsed, addsUsedExplicit, includeGoalies, noMatchup, manualBoosts, projectedAdds };
}
...
/**
 * Build multipliers from manual --boost flag.
 * Boosted stats get 2.5x, everything else stays at 1.0.
 */
function buildManualBoosts(boostAbbrs, leagueWeights) {
  // Map abbreviations back to stat IDs
  const abbrToId = {};
  for (const [statId, info] of Object.entries(YAHOO_STATS)) {
    abbrToId[info.abbr.toUpperCase()] = statId;
    // Also support common aliases
    abbrToId[info.name.toUpperCase()] = statId;
  }
  ...
  for (const abbr of boostAbbrs) {
    const statId = abbrToId[abbr];
    if (statId && leagueWeights.scoringStatIds.has(statId)) {
      multipliers[statId] = 2.5;
      resolved.push(YAHOO_STATS[statId].abbr);
    } else if (statId) {
      unresolved.push(`${abbr} (not a scoring stat in this league)`);
    } else {
      unresolved.push(`${abbr} (unknown stat)`);
    }
  }

  if (unresolved.length > 0) {
    console.log(`  Warning: skipped boosts: ${unresolved.join(', ')}`);
  }
  if (resolved.length > 0) {
    console.log(`  Boosting: ${resolved.join(', ')} (2.5x)`);
  }
...
async function run(config, cliArgs) {
  ...
  leagueKey = resolveLeague(parsed.leagueKey) || config.defaultLeague;
  if (!leagueKey) {
    console.error('No league specified. Pass a league key/alias or run: stream setup');
    process.exit(1);
  }
  if (parsed.leagueKey && !parsed.leagueKey.match(/^\d+\.l\.\d+$/) && leagueKey) {
    console.log(`Using league: ${leagueKey}`);
  }

  let { targetDate, addsUsed, addsUsedExplicit, includeGoalies, noMatchup, manualBoosts, projectedAdds } = parsed;
  ...
  // Determine category weighting strategy:
  // --no-matchup: skip matchup entirely, use raw league weights
  // --boost "HIT,BLK": manually boost specific stats
  // default: auto-detect from current matchup
  ...
  if (manualBoosts) {
    // Manual boost overrides matchup detection
    categoryWeights = buildManualBoosts(manualBoosts, leagueWeights);
    // Still fetch matchup for display if current week
    if (currentWeek && isCurrentWeek && !noMatchup) {
  ...
  } else if (!noMatchup && currentWeek && isCurrentWeek) {
    console.log('Fetching matchup data...');
    categoryNeeds = await fetchMatchupNeeds(teamKey, leagueKey, currentWeek, settings, leagueWeights);
