      ...
      if (item && item.managers) {
        for (const m of item.managers) {
          if (m.manager && m.manager.is_current_login === '1') {
            return { teamKey: key, teamName: name };
          }
        }
      }
    }
...
function buildStreamingPlan(scoredStreamers, days, addsRemaining, rosterSlots) {
  ...
  strategies.push({
    label: 'Full week',
    waves: [{ dates: futureDates, adds: addsRemaining }],
  });
  ...
  for (let splitIdx = 1; splitIdx < futureDates.length - 1; splitIdx++) {
    ...
    const wave1Adds = Math.floor(addsRemaining / 2);
    const wave2Adds = addsRemaining - wave1Adds;
  ...
  if (addsRemaining >= 3 && futureDates.length >= 3) {
    for (let s1 = 1; s1 < futureDates.length - 1; s1++) {
      for (let s2 = s1 + 1; s2 < futureDates.length; s2++) {
        ...
        const a1 = Math.floor(addsRemaining / 3);
        const a2 = Math.floor(addsRemaining / 3);
        const a3 = addsRemaining - a1 - a2;
        if (a1 > 0 && a2 > 0 && a3 > 0) {
          strategies.push({
            label: `3 waves`,
            waves: [
...
function printHeader(leagueSettings, teamName, weekDates, addsRemaining, maxAdds) {
  const leagueName = leagueSettings.meta?.name || leagueKey;
  const startDate = weekDates[0];
  const endDate = weekDates[weekDates.length - 1];
  console.log(`\nHockey Streaming Optimizer — ${leagueName}`);
  console.log(`${formatDate(startDate)} – ${formatDate(endDate)} | Team: ${teamName} | Adds: ${addsRemaining} of ${maxAdds} remaining\n`);
}
...
function printStreamingPlan(plan, addsRemaining) {
  console.log(`══ SUGGESTED STREAMING PLAN (${addsRemaining} adds) ══════════════════════════`);
  if (!plan) {
...
async function run(config, cliArgs) {
  ...
  if (!isCurrentWeekTarget && !addsUsedExplicit) {
    addsUsed = 0;
  }
  let addsRemaining = maxAdds - addsUsed;

  console.log('Finding your team...');
  const { teamKey, teamName } = await findMyTeamKey(leagueKey);
  ...
  console.log('');
  printHeader(settings, teamName, weekDates, addsRemaining, maxAdds);
