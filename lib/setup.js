...
function ask(rl, question) {
  return new Promise((resolve, reject) => {
    rl.question(question, answer => {
      if (answer === undefined) reject(new Error('Input closed (EOF or Ctrl+C)'));
      else resolve(answer);
    });
  });
}
...
async function runSetup() {
  ...
  try {
    ...
    ensureConfigDir();
    saveConfig({ clientId, clientSecret, defaultLeague: null });
    ...
    // Step 6: Update config with league
...
module.exports = { runSetup, runAuth };
